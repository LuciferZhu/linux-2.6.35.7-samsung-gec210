...
#ifndef CONFIG_FB_S3C_NR_BUFFERS
...
static struct s3c_platform_fb gec210_fb_data __initdata = {
	.hw_ver	= 0x62,
	.nr_wins = CONFIG_FB_S3C_NR_BUFFERS,
	.default_win = CONFIG_FB_S3C_DEFAULT_WINDOW,
	.swap = FB_SWAP_WORD | FB_SWAP_HWORD,
       .lcd = &wvga_s70,
...
#elif defined(CONFIG_FB_S3C)
static struct s3cfb_lcd lcd_AT070 = {
	.width = 800,
	.height =480,
	.p_width =  158,
	.p_height = 88,
...
void gec210_lcd_cfg_gpio(struct platform_device *dev)
{
	struct resource * gec210_lcd_res;
	unsigned int * gpf0con_va, * gpf1con_va, * gpf2con_va, * gpf3con_va;	

	dev_info(&dev->dev, "<%s>: --------enter-------\n", __FUNCTION__);
	
	/* GPF0CON */
	gec210_lcd_res = request_mem_region(0xE0200120, 4, "GPF0_LCD");
	if (gec210_lcd_res == NULL)
		dev_err(&dev->dev, "[err]: request_mem_region !");

	gpf0con_va = ioremap(0xE0200120,  4);
	if (gpf0con_va == NULL)
		dev_err(&dev->dev, "[err]: ioremap !");

	/* GPF1CON */
	gec210_lcd_res = request_mem_region(0xE0200140, 4, "GPF1_LCD");
	if (gec210_lcd_res == NULL)
		dev_err(&dev->dev, "[err]: request_mem_region !");

	gpf1con_va = ioremap(0xE0200140,  4);
	if (gpf1con_va == NULL)
		dev_err(&dev->dev, "[err]: ioremap !");
	
	/* GPF2CON */
	gec210_lcd_res = request_mem_region(0xE0200160, 4, "GPF2_LCD");
	if (gec210_lcd_res == NULL)
		dev_err(&dev->dev, "[err]: request_mem_region !");

	gpf2con_va = ioremap(0xE0200160,  4);
	if (gpf2con_va == NULL)
		dev_err(&dev->dev, "[err]: ioremap !");
	
	/* GPF3CON */
	gec210_lcd_res = request_mem_region(0xE0200180, 4, "GPF3_LCD");
	if (gec210_lcd_res == NULL)
		dev_err(&dev->dev, "[err]: request_mem_region !");

	gpf3con_va = ioremap(0xE0200180,  4);
	if (gpf3con_va == NULL)
		dev_err(&dev->dev, "[err]: ioremap !");

	/* config into video function */
	writel(0x22222222, gpf0con_va);
	writel(0x22222222, gpf1con_va);
	writel(0x22222222, gpf2con_va);
	writel( (readl(gpf3con_va) & ~0xffff) | 0x2222, gpf3con_va);

	/* mDNIe SEL: why we shall write 0x2 ? */
	/* 配置FIMD为RGB接口,相当于rDISPLAY_CONTROL=2<<0;
	*/		
	writel(0x2, S5P_MDNIE_SEL);
}
...
#else
		.modalias	 = "mpu6500_spi",	/* device node name */
...
#ifdef CONFIG_FB_S3C
	s3cfb_set_platdata(&gec210_fb_data);   /* �?003�?*/
#endif
...
#elif CONFIG_MACH_zxd_GEC210
MACHINE_START(zxd_GEC210, "zxd_GEC210")
#endif
